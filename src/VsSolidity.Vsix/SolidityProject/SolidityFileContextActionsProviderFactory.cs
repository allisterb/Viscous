...
{
    [ExportFileContextActionProvider((FileContextActionProviderOptions)VsCommandActionProviderOptions.SupportVsCommands, ProviderType, ProviderPriority.Normal, VsSolidityPackageIds.SolidityFileContextType)]
    public class SolidityFileContextActionProviderFactory : IWorkspaceProviderFactory<IFileContextActionProvider>, IVsCommandActionProvider
    {
        ...
        private static readonly Guid ProviderCommandGroup = VsSolidityPackageIds.GuidVsSolidityPackageCmdSet;
        private static readonly IReadOnlyList<CommandID> SupportedCommands = new List<CommandID>
        {
            new CommandID(VsSolidityPackageIds.GuidVsSolidityPackageCmdSet, VsSolidityPackageIds.Cmd1Id),
            new CommandID(VsSolidityPackageIds.GuidVsSolidityPackageCmdSet, VsSolidityPackageIds.Cmd4Id),
        };
            ...
            public Task<IReadOnlyList<IFileContextAction>> GetActionsAsync(string filePath, FileContext fileContext, CancellationToken cancellationToken)
            {
                return Task.FromResult<IReadOnlyList<IFileContextAction>>(new IFileContextAction[]
                {
                    new SolidityFileContextAction(
                        fileContext,
                        new Tuple<Guid, uint>(ProviderCommandGroup, VsSolidityPackageIds.Cmd1Id),
                    ...
                    new SolidityFileContextAction(
                        fileContext,
                        new Tuple<Guid, uint>(VsSolidityPackageIds.GuidVsSolidityPackageCmdSet, VsSolidityPackageIds.Cmd4Id),
                        ...
                        async (fCtxt, progress, ct) =>
                        {
                            ...
                            if (r != null)
                            {
                                var window = (SolidityStaticAnalysisToolWindow) await VsSolidityPackage.Instance.ShowToolWindowAsync(typeof(SolidityStaticAnalysisToolWindow), 0, true, VsSolidityPackage.Instance.DisposalToken);
