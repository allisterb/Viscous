...
namespace Viscous
{
    public class VSUtil : Runtime
    {
        ...
        protected static string[] LogWindowNames = new string[] { "Viscous" };
        ...
        public static void LogToViscousWindow(string text)
        {
            LogInfo("Viscous", text);    
            ActivateLogOutputPane("Viscous");
        }

        public static void LogViscousInfo(string text) => LogInfo("Viscous", text);

        public static void LogViscousError(string text) => LogError("Viscous", text);
        ...
        public static string LoadUserSettings(IServiceProvider serviceProvider, string propertyName, string defaultValue = null)
        {
            ...
            return settingsStore.GetString("Viscous", propertyName, defaultValue);
        }
        ...
        public static void SaveUserSettings(IServiceProvider serviceProvider, string propertyName, string value)
        {
            ...
            settingsStore.CreateCollection("Viscous");
            settingsStore.SetString("Viscous", propertyName, value);
        }

        public static void ShowModalErrorDialogBox(string text, string title = null)
        {
            ThreadHelper.ThrowIfNotOnUIThread();
            VsShellUtilities.ShowMessageBox(ViscousPackage.Instance, text, title ?? "Viscous error:",
        ...
        {
            ThreadHelper.ThrowIfNotOnUIThread();
            var dte = ViscousPackage.Instance.GetService<EnvDTE.DTE, EnvDTE80.DTE2>();
            if (dte == null)
            {
        ...
        {
            ThreadHelper.ThrowIfNotOnUIThread();
            var dte = ViscousPackage.Instance.GetService<DTE, EnvDTE80.DTE2>();
            if (dte == null)
            {
                ShowModalErrorDialogBox("Could not get DTE service.");
                return null;
        ...
        public IProjectLockService GetProjectLockService()
        {
            ...
            IComponentModel componentModel = ViscousPackage.Instance.GetService<SComponentModel, IComponentModel>();
        ...
        {
            ThreadHelper.ThrowIfNotOnUIThread();
            var solutionService = ViscousPackage.Instance.GetService<SVsSolution, IVsSolution>();
