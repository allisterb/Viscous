    ...
    public class SolidityLanguageClient : Runtime, ILanguageClient, ILanguageClientCustomMessage2
    {
        ...
        public async Task<Connection> ActivateAsync(CancellationToken token)
        {
            ...
            if (!VSUtil.VSServicesInitialized)
            {
                ...
                {
                    VSUtil.LogInfo("Viscous", "Viscous package services initialized.");
                }
                else
                {
                    Error("Could not initialize Viscous package services.");
                    return null;
                }
            }
            if (File.Exists(LanguageServerPath))
            {
               VSUtil.LogInfo("Viscous", "Solidity language server present.");
            }
            else
            {
                VSUtil.ShowLogOutputWindowPane(ServiceProvider.GlobalProvider, "Viscous");
                VSUtil.LogInfo("Viscous", "Installing Solidity language server...");
                ...
                if (File.Exists(LanguageServerPath))
                {
                    VSUtil.LogInfo("Viscous", "Solidity language server installed.");
                }
                else
                {
                    var err = output.ContainsKey("stderr") ? (string)output["stderr"] : Runtime.GetRunCmdError(output);
                    VSUtil.LogError("Viscous", $"Could not install the Solidity language server using '{AppSettings.JSPackageManagerCmd}'. Check that it is installed and on your PATH. {err}");
                    return null;
                }
            }
