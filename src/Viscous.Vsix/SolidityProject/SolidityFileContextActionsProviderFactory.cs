...
namespace Viscous
{
    [ExportFileContextActionProvider((FileContextActionProviderOptions)VsCommandActionProviderOptions.SupportVsCommands, ProviderType, ProviderPriority.Normal, ViscousPackageIds.SolidityFileContextType)]
    public class SolidityFileContextActionProviderFactory : IWorkspaceProviderFactory<IFileContextActionProvider>, IVsCommandActionProvider
    {
        ...
        private static readonly Guid ProviderCommandGroup = ViscousPackageIds.GuidViscousPackageCmdSet;
        private static readonly IReadOnlyList<CommandID> SupportedCommands = new List<CommandID>
        {
            new CommandID(ViscousPackageIds.GuidViscousPackageCmdSet, ViscousPackageIds.Cmd1Id),
            new CommandID(ViscousPackageIds.GuidViscousPackageCmdSet, ViscousPackageIds.Cmd4Id),
        };
            ...
            public Task<IReadOnlyList<IFileContextAction>> GetActionsAsync(string filePath, FileContext fileContext, CancellationToken cancellationToken)
            {
                return Task.FromResult<IReadOnlyList<IFileContextAction>>(new IFileContextAction[]
                {
                    new SolidityFileContextAction(
                        fileContext,
                        new Tuple<Guid, uint>(ProviderCommandGroup, ViscousPackageIds.Cmd1Id),
                    ...
                    new SolidityFileContextAction(
                        fileContext,
                        new Tuple<Guid, uint>(ViscousPackageIds.GuidViscousPackageCmdSet, ViscousPackageIds.Cmd4Id),
                        ...
                        async (fCtxt, progress, ct) =>
                        {
                            ...
                            if (r != null)
                            {
                                var window = (SolidityStaticAnalysisToolWindow) await ViscousPackage.Instance.ShowToolWindowAsync(typeof(SolidityStaticAnalysisToolWindow), 0, true, ViscousPackage.Instance.DisposalToken);
